...
def build_application(operation_mode_service=None, joystick_port=None):
    ...
    if _is_local_manual(operation_mode_service):
        return _build_local_manual_application(
            operation_mode_service=operation_mode_service,
            joystick_port=joystick_port
        )
    return _build_standard_application(
        operation_mode_service=operation_mode_service
    )


def _build_local_manual_application(operation_mode_service=None,
                                    joystick_port=None):
    ...
    # LOCAL + MANUAL exposes REST queries, but deliberately does not construct
    # Ev3Dev2MotorGateway, DriveService or any monitor/command queue.
    ...
    if joystick_port is not None:
        ...
        # Joystick must close before the owned motor session is finally closed.
...
def _build_standard_application(operation_mode_service=None):
